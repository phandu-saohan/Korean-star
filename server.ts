...
// API: Zalo OAuth v4 Callback Redirect Handler
app.get("/api/zalo/oauth-callback", async (req, res) => {
  try {
    ...
    console.error("[Zalo OAuth Callback Express Error]:", err);
    return res.status(500).send(`<h3>Lỗi xử lý Zalo OAuth Callback: ${err.message}</h3>`);
  }
});
